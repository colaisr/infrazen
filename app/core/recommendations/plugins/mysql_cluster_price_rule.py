"""
MySQL Managed Cluster Price Comparison Rule

Compares Yandex Managed MySQL clusters with Selectel/Beget Managed MySQL.
Uses Selectel unified DBaaS pricing (same as PostgreSQL).
"""
...
class MySQLClusterPriceCheckRule(BaseRule):
    """Compare managed MySQL cluster costs across providers."""
    
    @property
    def id(self) -> str:
        return "cost.price_check.mysql_cluster"
    
    @property
    def name(self) -> str:
        return "Сравнение стоимости MySQL кластера"
    
    @property
    def description(self) -> str:
        return (
            "Сравнивает стоимость управляемого MySQL кластера "
    ...
    def resource_types(self):
        return {"mysql-cluster", "mysql_cluster", "mariadb-cluster"}
    
    def applies(self, resource: Resource, context) -> bool:
        """Applies only to MySQL cluster resources."""
        try:
    ...
    def evaluate(self, resource: Resource, context) -> List[RecommendationOutput]:
        """Compare current MySQL cluster with alternatives."""
        ...
        # Find alternatives using the same helper as PostgreSQL
        ...
        title = "Доступен более дешёвый managed MySQL у другого провайдера"
        
        desc = (
            f"MySQL кластер '{cluster_name}' ({total_vcpus} vCPU, {total_ram_gb:.0f} GB RAM, {total_storage_gb:.0f} GB) "
    ...
    def _find_alternatives(self, candidate_providers, cluster_geo_prefix, 
                          total_vcpus, total_ram_gb, total_storage_gb, 
                          total_hosts, current_monthly):
        """Find alternatives across providers (Selectel and Beget)."""
        alternatives = []
        
        for target_provider in candidate_providers:
            if target_provider in ['selectel', 'beget']:
                # Both use the same matching logic
                # For Selectel, we use postgresql-cluster pricing (unified DBaaS)
                # For Beget, we use mysql-cluster pricing (specific to MySQL)
                resource_type = 'mysql-cluster' if target_provider == 'beget' else 'postgresql-cluster'
                ...
                for region in regions:
                    match = ProviderPrice.query.filter(
                        ProviderPrice.provider == target_provider,
                        ProviderPrice.resource_type == resource_type,
...
RULES = [MySQLClusterPriceCheckRule]
